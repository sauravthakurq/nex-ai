...
export function SceneSidebar({
  ...
  return (
    <>
      <div 
        className={cn(
          "md:hidden fixed inset-0 z-[140] bg-black/20 backdrop-blur-sm transition-opacity",
          collapsed ? "opacity-0 pointer-events-none delay-0" : "opacity-100"
        )}
      ...
      <div
        ...
        className={cn(
          "bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-r border-gray-100 dark:border-gray-800 shadow-[2px_0_24px_rgba(0,0,0,0.02)] flex flex-col shrink-0 overflow-visible transition-transform duration-150 z-[150]",
          "max-md:fixed max-md:inset-y-0 max-md:left-0 max-md:h-full max-md:!w-[90%]",
          collapsed ? "max-md:-translate-x-full md:relative" : "max-md:translate-x-0 md:relative"
        )}
      >
        {/* Drag handle */}
      {!collapsed && (
        <div
          ...
          className="absolute right-0 top-0 bottom-0 w-1.5 cursor-col-resize z-50 group hover:bg-gray-700/30 dark:hover:bg-black/30 active:bg-gray-900/40 dark:active:bg-gray-900/40 transition-colors"
        >
          <div className="absolute right-0.5 top-1/2 -translate-y-1/2 w-0.5 h-8 rounded-full bg-gray-300 dark:bg-gray-600 group-hover:bg-gray-700 dark:group-hover:bg-gray-900 transition-colors" />
        </div>
      )}

      <div className={cn('flex flex-col w-full h-full overflow-hidden transition-opacity duration-150', collapsed ? 'md:hidden max-md:opacity-0 delay-0' : 'opacity-100 delay-0')}>
        {/* Logo Header */}
        <div className="h-10 flex items-center justify-between shrink-0 relative mt-3 mb-1 px-3">
          ...
          >
            <div className="flex items-center gap-2">
              <div className="flex h-6 w-6 items-center justify-center rounded-lg border border-white/10 bg-black text-[9px] font-semibold tracking-[0.22em] text-white dark:border-white/15 dark:bg-white dark:text-black">
                N
              </div>
              <span className="text-[11px] font-semibold uppercase tracking-[0.26em] text-foreground">
                NEX AI
              </span>
            </div>
          </button>
          ...
          {scenes.map((scene, index) => {
            ...
            return (
              <div
                ...
                className={cn(
                  'group relative rounded-lg transition-all duration-100 cursor-pointer flex flex-col gap-1 p-1.5',
                  isActive
                    ? 'bg-blue-50 dark:bg-blue-900/20 ring-1 ring-blue-300 dark:ring-blue-800'
                  ...
                  <div className="flex items-center gap-2 max-w-full">
                    <span
                      className={cn(
                        ...
                        isActive
                          ? 'bg-blue-500 dark:bg-blue-600 text-white shadow-sm shadow-blue-500/30'
                ...
                <div className="relative aspect-video w-full rounded overflow-hidden bg-gray-100 dark:bg-gray-800 ring-1 ring-black/5 dark:ring-white/5">
                  <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    {isSlide && (
                      <div
                        className={cn(
                          'absolute inset-0 bg-gray-900/0 transition-colors',
                          isActive
                            ? 'bg-gray-900/0'
            ...
            (() => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    'group relative rounded-lg flex flex-col gap-1 p-1.5 transition-all duration-100',
                    isFailed
                    ...
                    isActive &&
                      !isFailed &&
                      'bg-gray-100 dark:bg-black/20 ring-1 ring-gray-300 dark:ring-black opacity-100',
                  )}
                >
                  {/* Scene Header */}
                  <div className="flex justify-between items-center px-2 pt-0.5">
                    <div className="flex items-center gap-2 max-w-full">
                      <span
                        className={cn(
                          'text-[10px] font-black w-4 h-4 rounded-full flex items-center justify-center shrink-0',
                          isActive && !isFailed
                            ? 'bg-black dark:bg-gray-900 text-white shadow-sm shadow-gray-900/30'
