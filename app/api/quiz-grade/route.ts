...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const systemPrompt = isZh
      ? `You are a professional educational assessment expert。请根据Question和Student answer进行评Points并给出简短评语。
Must reply with JSON Format for reply（Do not include anything else）：
{"score": <0to${points}integer out of>, "comment": "<1-2 sentences of feedback>"}`
...
{"score": <integer from 0 to ${points}>, "comment": "<one or two sentences of feedback>"}`;
    ...
    const userPrompt = isZh
      ? `Question：${question}
满Points：${points}Points
${commentPrompt ? `评Points要点：${commentPrompt}\n` : ''}Student answer：${userAnswer}`
      : `Question: ${question}
